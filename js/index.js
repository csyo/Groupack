$(function (e) {
	// 處理完成的任務
	if (JSON.parse(localStorage.task_done || '[]').length) {
		taskdone();
	}
	// 測試視窗大小
	$('.testdevice').hide();
		...
		if (!$(this).hasClass('page_btn_now')) {
			...
			$.ajax({
				...
				data: {
					...
					sendgroupid: localStorage.group_selected
				},
				error: function (xhr) {
...
function save_user() {
	$.post('http://chding.es.ncku.edu.tw/Groupack/saveUser.php', {
		fbid: localStorage.FB_id,
		fbname: localStorage.FB_name
	})
...
function id_name() {
	$.post('http://chding.es.ncku.edu.tw/Groupack/getNames.php')
		.done(function (r) {
			...
			localStorage.setItem("id_name", JSON.stringify(names));
		})
		.fail(function (e) {
			console.log(e)
		});
...
function taskdone() {
	...
	$.post("http://chding.es.ncku.edu.tw/Groupack/todolist/taskdone.php", {
		data: localStorage.task_done,
		fbid: localStorage.FB_id
	})
		.fail(function (xhr) {
			console.log(xhr);
