	            ...
	            if ( item.w_id === id ) {
	                ...
	                $('div.workspace_user span[role=time]').text( item.createdTime );
				        // 取得此 workspace 所有 folder
							if ( item.folders ) {
								$('div.noFolder').remove();
								for (var i = 0, folder; folder = item.folders[i]; i++)     
								  showFolder( folder.f_id, folder.f_name, folder.f_comment, folder.createdTime, folder.creatorID );   
							} else {
								if( $('div.noFolder') )
	...
	if ( name ) {
		...
		var item = ''+
			'<li class="w-item" id="'+ id +'">'+
				'<a class="co_a" title="'+ name +'">'+ name + '</a>' +
			'</li>';
...
function addWorkspace( name , comment ){
    ...
    // 傳送到資料庫
    $.post( "db/setWorkspace.php",
    	{ gid: gID, wid: wID, wname: name, comment:comment , fbid:localStorage.FB_id })
...
function updateWorkspace(data){
	...
	if( workspaces ) {
		for (var i = workspaces.length - 1; i >= 0; i--) {
			div += '<li class="w-item"> <a class="co_a" id="'+ workspaces[i].w_id +'">'+ workspaces[i].w_name + '</a>';
		};
