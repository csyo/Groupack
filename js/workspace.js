...
// 顯示介面: 群組共享 ( Workspace )
$(document).on('click','#inline_workspace_cards div.workspace_cards_content_inf_field > div.workspace_cards_content_inf_field_content:nth-child(1)', function(){
	$(this).addClass('this_sharing_with_group_on');
	$('#sharing_with_group').addClass('sharing_with_group_on').removeClass('dom_hidden').css( {top : '', left : ''} );
	$('#sharing_with_group_background').removeClass('dom_hidden').css( {top : '', left : ''} );
	showWS();
});
...
$(function(){
    ...
    $('#Workspace_nav').on('click', 'li.w-item > a', function(){
		...
		$('#myWorkspace').addClass('workspace_on').css( {marginLeft : '', marginRight : ''} ).removeClass('dom_hidden').attr('style', '');
		$('#wrapper').css( {marginLeft:'',marginRight:''} ).addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('#under-footer').css( {marginLeft : '', marginRight : ''} ).addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('#timeline_wrapper').css( {marginLeft:'',marginRight:''} ).addClass('dom_hidden').offset({ top: -1000, left: -1000 });
    });
    
    // 離開介面: 離開 Workspace 中所有 Folders
    $('#delete_workspace').click(function(){ workspaceLeave(); });
	...
	$('#myWorkspace').on('click', 'a.workspace_folder_area', function(){
		/* 處理資料 */
		var folderID = this.parentNode.parentNode.id;
		localStorage.setItem("folder_selected", folderID);
		// 取得所有 Card
		$.post( "http://chding.es.ncku.edu.tw/Groupack/workspace/getCards.php" , { fid: folderID } )
		.fail( function(x,e,txt) { console.log(txt)} )
		.done( function(r) { console.log("成功取得 Card 資料 from "+folderID) 
	...
	$('#cards_area_leave').click(function(){
		...
		$('#card_wrapper_background').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('#card_wrapper').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('div.cards_area').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('div.cards_area_header').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
...
function Enter_W(e){
    var id = $(e).attr('id');  // 取得 wID
	console.log('選中:'+id);
    localStorage.setItem('workspace_selected', id);
    
    if( id ){
        $('div.workspace_four_column').remove(); // 清空之前的項目
        var wdata = JSON.parse( sessionStorage.w_data ); // 取得 worksapce 暫存資料
        if ( wdata.length ) {
        	for (var i = 0, item; item = wdata[i]; i++) {
	            if ( item.wid === id ) {
	                $('div.workspace_comment_inf span').text( item.text );
	                $('div.workspace_user div').attr( 'role' , item.creator ).text( getName(item.creator) );
	                $('div.workspace_user span[role=time]').text( item.time );
	                break;
	            }
        	}
        }
        // 取得此 workspace 所有 folder 資料s
        $.post( "http://chding.es.ncku.edu.tw/Groupack/workspace/getFolders.php", { wid: id })
        .fail( function(x,e,txt) { console.log(txt); } )
        .done( function(r) {
            var data = r === '[]' ? '': JSON.parse(r);
            if ( data ) {
                $('div.noFolder').remove();
                for (var i = 0, item; item = data[i]; i++)     
                    showFolder( item.fid, item.fname, item.ftext, item.ftime, item.id );   
            } else {
            	if( $('div.noFolder') )
...
function showFolder( fid , name , comment , time , creator ) {
	// 新增 folder 調整參數
...
function newWorkspace(){    
	...
	if ( name ) {
		...
		// 新增到 DOM
		var item = '\
			<div class="mySidebar_container_workspace_area_item"> \
				<div class="mySidebar_container_workspace_area_item_text" id="'+id+'">'+name+'</div> \
			</div>';
		$('div.mySidebar_container_workspace_area').prepend( item );
...
function addWorkspace( name , comment ){
    ...
    // 傳送到資料庫
    $.post( "http://chding.es.ncku.edu.tw/Groupack/workspace/setWorkspace.php", { gid: gID, wid: wID, wname: name, comment:comment , fbid:localStorage.FB_id })
    .fail( function(e) { console.log(e); })
    .done( function(r) { console.log(r); console.log('addWorkspace: T_savedata');
...
function editWorkspace(){ 
	...
	// 傳送到資料庫
	$.post("http://chding.es.ncku.edu.tw/Groupack/workspace/setWorkspace.php", { gid: gid , wid: wid , wname: name , comment:comment , fbid: null })
	.fail( function(e) {console.log(e)})
	.done( function(r) { console.log(r); console.log('editWorkspace: T_savedata');
		if( localStorage.T_savedataTimeout ){
			window.setTimeout( function(){
				var a = window.setTimeout( function(){
					T_savedata('none', 'none', wid, 'none');
...
/*
	$('#slider_btn').click(function(){
		...
		if ( localStorage.group_selected ){
			// 清理之前的項目
			$('div.mySidebar_container_workspace_area_item').remove();        
			
			// 加入目前的項目
			$.post( "http://chding.es.ncku.edu.tw/Groupack/workspace/getWorkspaces.php", { gid: localStorage.group_selected } )
			.fail( function(x,e,txt) { console.log(txt)} )
			.done( function(r) {
				var data = JSON.parse(r); console.log("取得 Workspace: "+localStorage.group_selected);
				var wdata = [];
				var div = '';
				for (var i = 0, item; item = data[i]; i++) {
					div += '\
						<div class="mySidebar_container_workspace_area_item">\
							<div class="mySidebar_container_workspace_area_item_text" id="'+item.id+'">'+item.name+'</div>\
						</div>';
					// 儲存該項目資料
					wdata.push({ wid: item.id, text: item.text, creator: item.creator, time: item.time });
					sessionStorage.setItem( "w_data" , JSON.stringify(wdata) );
				}
				$('div.mySidebar_container_workspace_area').prepend(div);
			});
		}
	});*/
});

// 載入完後，顯示群組所有 Workspaces
...
function newFolder(){
	...
	if ( name ) {
	    ...
	    ( $('#sharing_with_group').hasClass('sharing_with_group_on') ) ? addToSharing(id,name) : showFolder(id,name,comment); // 新增到 群組共享 or WS介面
...
function addFolder( name, comment ){
    ...
    var wID = localStorage.workspace_selected;
    // 傳送到資料庫
    $.post( "http://chding.es.ncku.edu.tw/Groupack/workspace/setFolder.php", { wid: wID , fid: folderID , fname: name , comment: comment , fbid: localStorage.FB_id })
    .fail( function(x,e,txt) { console.log(txt)} )
...
function editFolder(){
    ...
    // 傳送到資料庫
    $.post("http://chding.es.ncku.edu.tw/Groupack/workspace/setFolder.php", { wid: wid , fid: fid , fname: name , comment: comment , fbid: null })
    .fail( function(x,e,txt) { console.log(txt)} )
    .done( function(r) { console.log(r); console.log('editFolder: T_savedata');
		if( localStorage.T_savedataTimeout ){
			window.setTimeout( function(){
				var a = window.setTimeout( function(){
...
function newCard(){
	var fID = localStorage.folder_selected;
	var name = $('#inline_card_wrapper_name').val();
	var content = $('#inline_card_wrapper_comment').val();
	var url = $('#inline_card_wrapper_url').val();
...
if ( name && url ) {
	...
	// 傳送到資料庫
	sendCard( localStorage.FB_id , cID , fID , name , content , url , time );
...
// 傳送單個 Card 資料
function sendCard( id, cid , fid , title , content , url ,time ){
    $.post("http://chding.es.ncku.edu.tw/Groupack/workspace/setCard.php",{ fbid: id , cid: cid , fid: fid , title: title , content: content , url: url , time:time })
    .fail( function(x,e,txt) { console.log(txt)} )
    .done( function(r) { console.log(r); console.log('sendCard: T_savedata');
		if( localStorage.T_savedataTimeout ){
			window.setTimeout( function(){
				var a = window.setTimeout( function(){
...
function removeById( xid ) {
    $.post( "http://chding.es.ncku.edu.tw/Groupack/workspace/removeById.php", { xid: xid })
    .fail( function(x,e,txt) { console.log(txt); } )
    .done( function(r) { console.log(r); console.log('removeById: T_savedata');
		if( localStorage.T_savedataTimeout ){
			window.setTimeout( function(){
				var a = window.setTimeout( function(){
...
// 設定頁面在哪
function SetPage(){	 console.log('SetPage');
...
function getNow(){
	...
	var hour = d.getHours(); var minute = d.getMinutes(); var second = d.getSeconds(); 
