...
function tag_AddToTag() { //確定加上 Tag： 打開我的標籤時
    ...
    if (temp == 0) { //判斷 Tag 是否使用過
        ...
        $('div.tag[sid=' + d + ']').find('div.tag_content_color[t_color=' + a.attr('t_color') + ']').css({
            'background': a.attr('t_color'),
            'left': b * 5
        });
        $.colorbox.close();
    } else {
...
function getTagData() {
    ...
    $.post('db/t_get.php', {
        ...
        .done(function(r) {
            // data structure:
            // [ Object, Object, ... ]
            // Object : { tid: ..., name: ..., color: ..., cards: Array }
            ...
            data.forEach(function(tag) {
                ...
                $('div.all_tags_inf[t_sid=' + tag.tid + ']').css('background', tag.color);
...
function tagDivGen(tag) {
    var div = '<div class="all_tags_inf" ' + 't_sid="' + tag.tid + '" tid="' + tag.tid + '" t_color="' + tag.color + '" t_name="' + tag.name + '" style="background: ' + tag.color + '">' + '<div class="all_tags_inf_ModifyTag_area">' + '<a class="co_a all_tags_inf_ModifyTag" href="#inline_all_tags_inf_ModifyTag">&nbsp;</a>' + '</div>' + '<div class="all_tags_inf_icon"></div>' + '<div class="all_tags_inf_text">' + tag.name + '</div>' + '</div>', // 產生 Tag DIV
        tags = JSON.parse(sessionStorage.tags_color || '[]'); // 暫存已使用過的 Tag 色
