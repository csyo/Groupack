$(function(){
	...
	$('#leave_tag').click(function(){    //離開 我的標籤介面
		...
		$('#tag_wrapper_background').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('#tag_wrapper').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('#tag_wrapper div.tag_area').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
		$('#tag_wrapper div.tag_header').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
	});
	...
	$('div').on('click', function(){      //關閉 管理標籤選單
		if( $('#show_manager_tags').hasClass('show_manager_tags_on') ){
			...
			$('div.tag_manager').offset({ top: -1000, left: -1000 });
		}
	...
	$('#inline_tag_manager_addtag div.inline_addtag_wrapper_submit > input').click(function(){    //確定 新增 tag
		var a = $('#inline_addtag_wrapper_color').attr('color');
		var b = new Array();
		b = tag_getTime().split(' ');
		//判斷顏色是否用過
		...
		if( ( c === 0 && temp === 0 ) || used === 0 ){    //只考慮底色 色調、濃度、亮度皆小於187 的並且顏色未使用過
			if( $('#inline_addtag_wrapper_name').val() ){    //判斷是否輸入名稱
				$('#inline_all_tags').children('.all_tags_ShowAll').after(
					'<div class="all_tags_inf" t_sid="'+b[0]+'" t_color="'+a+'" t_name="'+$('#inline_addtag_wrapper_name').val()+'">'+
				...
				$('div.all_tags_inf[t_sid='+b[0]+']').css('background', a);
				$.fancybox.close();
				newTag( b[0] );
			...
			alert('請選擇色調、濃度、亮度皆小於187的顏色，並且Tag顏色不得重覆');
		}
	});
	...
	$('#inline_AddToTag_wrapper_name').change(function(){    //AddToTag 自動改變選單顏色
		var a = $('#inline_AddToTag_wrapper_name option:selected').attr('t_color');
		$('#inline_AddToTag_wrapper_color').css('background', a);
	});
			...
			afterClose: function(){
				...
				if( a ){
					...
					for( var i = 0; i < a; i++ ){
						...
						b += '<option t_sid="'+$c.attr('t_sid')+'" t_color="'+$c.attr('t_color')+'" t_name="'+$c.attr('t_name')+'" tid="'+$c.attr('tid')+'">'+$c.attr('t_name')+'</option>'
					}
					$('#inline_AddToTag_wrapper_name').append(b);
					//AddToTag 預設 顯示選單第一個的顏色
					var c = $('#inline_AddToTag_wrapper_name > option:nth-child(1)').attr('t_color');
					$('#inline_AddToTag_wrapper_color').css('background', c);
					$('#cboxOverlay').show();
					$('#colorbox').show();
				}
			}
	...
	$('#inline_all_tags_inf_ModifyTag div.inline_ModifyTag_wrapper_submit > input').click(function(){    //確定 修改 tag
		if( $('#inline_ModifyTag_wrapper_name').val() ){    //判斷是否輸入名稱
			...
			$('div.all_tags_inf[t_sid='+m+']')
					...
					.attr('t_color', $('#inline_ModifyTag_wrapper_color').attr('color'))
					.css('background', $('#inline_ModifyTag_wrapper_color').attr('color'))
			...
			for( var i = 0; i < a; i++ ){
				...
				if( f >= 0 ){    //沒找到要修改的地方 則不執行修改
					var g = ''; var h = ''; var k = '';    //重新組合陣列，除了要刪除的資訊之外
					for( var j = 0; j < c.length; j++ ){
						if( j != f ){
							...
							g += ' '+l[0];
							h += ' '+$('#inline_ModifyTag_wrapper_color').attr('color');
							k += ' '+$('#inline_ModifyTag_wrapper_name').val();
						}	
					}
					//.substr(1) 刪除第一個空白
					b.attr('t_sid', g.substr(1));
					b.attr('t_color', h.substr(1));
		...
		for( var i = 0; i < k; i++ ){
			...
			if( d >= 0 ){    //有找到要刪除的資訊才執行
				if( a.length > 1 ){    //判斷card被加上的標籤個數
					var e = ''; var f = ''; var g = '';    //重新組合陣列，除了要刪除的資訊之外
					...
					l.attr('t_color', f.substr(1));
					l.attr('t_name', g.substr(1));
					//刪除要刪除的資訊
					l.find('div.tag_content_color[t_color='+b[d]+']').remove();
					for( var j = 0; j < (a.length -1); j++ ){
						l.find('div.tag_content_color:nth-child('+(j+1+1)+')').css('left', j*5);
					}
	...
	*/
	$('div.color_field').on( 'click', 'input', function tag_color() {	
		var color = colorSelect();		
//		while( $.inArray( color , JSON.parse(sessionStorage.tags_color) ) !== -1 )
//			color = colorSelect();	
...
$(document).on('click', '#tag_wrapper div.tag_content_inf', function(){  // 我的標籤介面：顯示 管理card選單  
	...
	if( !$(this).hasClass('tag_content_inf_on') ){
		...
		info = { title: title , url: url , content: content };
...
$(document).on('click', '#inline_all_tags a.all_tags_inf_ModifyTag', function(){  // 在所有的 tags 介面  點擊 修改 tag
	...
	$('#colorbox').hide();
	$('#cboxOverlay').hide();
	$('#inline_ModifyTag_wrapper_color').css('background',''+$(this).parents('div.all_tags_inf').attr('t_color')+'');
...
$(document).on('click', '#all_tags_inf div.all_tags_inf', function(){  // 在所有的 tags 介面  選擇 tag
	...
	if( !test ){
		...
		$('div.tag_container')
				...
				.attr('tag_color', $(this).attr('t_color'))
				.attr('tag_name', $(this).attr('t_name'));
		$('#tag_container_CurrentTag').css('color', $(this).attr('t_color')).text( $(this).attr('t_name') ).removeClass('dom_hidden');
		$.colorbox.close();	
	}
});
$(document).on('click', '#all_tags_inf div.all_tags_ShowAll', function(){  // 在所有的 tags 介面  選擇 顯示所有 tag
...
$(document).on('click', '#tag_wrapper div.tag_content_tagname > span', function(){  // 點擊 標籤卡片上方標籤: 移除標籤
	...
	if( a.length > 1){  // 判斷被加上的標籤個數
		...
		//.substr(1) 刪除第一個空白
		$a.attr('t_sid', e.substr(1)).attr('t_color', f.substr(1)).attr('t_name', g.substr(1))
				.find('div.tag_content_color[t_color='+b[d]+']').remove();  // 刪除要刪除的資訊
		for( var i = 0; i < (a.length -1); i++ ){
			$a.find('div.tag_content_color:nth-child('+(i+1+1)+')').css('left', i*5);
		}
...
$(document).on('click', '#tag_wrapper div.tag_content_inf_field > div.tag_content_inf_field_content:nth-child(1)', function(){  // 點擊標籤卡片: 顯示 群組共享
	...
	$('#sharing_with_group').addClass('sharing_with_group_on').removeClass('dom_hidden').css({'left':'', 'top':''});
	$('#sharing_with_group_background').removeClass('dom_hidden').css({'left':'', 'top':''});
});
...
function search_AddToTag(){    //確定加上 Tag： 一般情況
	...
	var b = addTag();
	var $a = $('div.this_search_on').next();
	$('#tag_container_CurrentTag').after(
		'<div class="tag" t_sid="'+a.attr('t_sid')+'" t_color="'+a.attr('t_color')+'" t_name="'+a.attr('t_name')+'" sid="'+b+'">'+
			'<div class="tag_content">'+
				'<div class="tag_content_tagname">'+
					'<span t_sid="'+a.attr('t_sid')+'">'+a.text()+'</span>'+
				'</div>'+
				'<div class="tag_content_color" t_color="'+a.attr('t_color')+'">&nbsp;</div>'+
				'<div class="tag_content_selectarea" title="選擇">'+
					'<div class="tag_content_selectarea_icon"></div>'+
				'</div>'+
				'<div class="tag_content_inf"></div>'+
				'<div class="tag_content_inf_field">'+
					'<div class="tag_content_inf_field_content" title="群組共享">'+
						'<div class="post_group">&nbsp;</div>'+
						'<div class="post_group_text">群組共享</div>'+
					'</div>'+
					'<div class="tag_content_inf_field_content" title="加上標籤">'+
						'<a class="co_a AddToTag" href="#inline_AddToTag" onclick="show_AddToTag()">&nbsp;</a>'+
						'<div class="post_tag">&nbsp;</div>'+
						'<div class="post_tag_text">加上標籤</div>'+
					'</div>'+
					'<div class="tag_content_inf_field_content" title="分享">'+
						'<div class="post_share">&nbsp;</div>'+
						'<div class="post_share_text">分享</div>'+
					'</div>'+
				'</div>'+
				'<div class="tag_content_area">'+
					'<div class="show_tag_content_area">'+
						'<h2 class="co_h2"><a class="co_a click_workspace_cards" href="'+
						$a.find('h2 > a').attr('href')
						+'" target="_blank" onclick="show_cardcontain()">'+
						$a.find('h2').text()
						+'</a></h2>'+
						'<p>'+$a.find('p').text()+'</p>'+
						'<div class="post_url">'+
							'<span>'+$a.find('span').text()+'</span>'+
						'</div>'+
					'</div>	'+
				'</div>'+
			'</div>'+
	...
	$('div.tag[sid='+b+']').find('div.tag_content_tagname > span[t_sid='+a.attr('t_sid')+']').css('background', a.attr('t_color'));
	$('div.tag[sid='+b+']').find('div.tag_content_color[t_color='+a.attr('t_color')+']').css('background', a.attr('t_color'));
	$.colorbox.close();
}
function workspace_AddToTag(){    //確定加上 Tag： 打開工作空間時
	var a = $('#inline_AddToTag_wrapper_name option:selected');
	var b = addTag();
	var $a = $('div.this_workspace_on').next();
	$('#tag_container_CurrentTag').after(
		'<div class="tag" t_sid="'+a.attr('t_sid')+'" t_color="'+a.attr('t_color')+'" t_name="'+a.attr('t_name')+'" sid="'+b+'">'+
			'<div class="tag_content">'+
				'<div class="tag_content_tagname">'+
				...
				'<div class="tag_content_area">'+
					'<div class="show_tag_content_area">'+
						'<h2 class="co_h2"><a class="co_a click_workspace_cards" href="'+
						$a.find('h2 > a').attr('href')+'" target="_blank" onclick="show_cardcontain()">'+
						$a.find('h2').text()
						+'</a></h2>'+
						'<p>'+$a.find('p').text()+'</p>'+
						'<div class="post_url">'+
...
function tag_AddToTag(){    //確定加上 Tag： 打開我的標籤時
	...
	if( temp == 0 ){    //判斷 Tag 是否使用過
		...
		$('.this_tag_on').parents('div.tag')
				...
				.attr('t_color', $a.attr('t_color')+' '+a.attr('t_color'));
		$('.this_tag_on').prevAll('div.tag_content_tagname')
				.append('<span t_sid="'+a.attr('t_sid')+'">'+a.text()+'</span>')
			.end()
				.parent('div.tag_content').children('div.tag_content_color:last').after('<div class="tag_content_color" t_color="'+a.attr('t_color')+'">&nbsp;</div>');
		$('div.tag[sid='+d+']').find('div.tag_content_tagname > span[t_sid='+a.attr('t_sid')+']').css('background', a.attr('t_color'));
		$('div.tag[sid='+d+']').find('div.tag_content_color[t_color='+a.attr('t_color')+']').css({'background':a.attr('t_color'), 'left':b*5});
		$.colorbox.close();
	}else{
...
function edit_tag(){  // 修改 Tag
	...
	var color = $('#inline_ModifyTag_wrapper_color').attr('color');
	var tid = $('#inline_all_tags .tag_modify_on').parents('.all_tags_inf').attr( 'tid' );

	// 傳送資料
	$.post( 'http://chding.es.ncku.edu.tw/Groupack/tag/set.php' , { fbid: localStorage.FB_id , name: name , tid: tid ,  color: color } )
...
function newTag( t_sid ){  // 新增 Tag
	...
	var color = $('#inline_addtag_wrapper_color').attr('color');
	var tid = 't'+createID();
...
function deleteTag( tid ){
	...
	$.post( 'http://chding.es.ncku.edu.tw/Groupack/tag/delete.php' , { tid: tid })
	.fail( function(x) { console.log(x) })
	.done( function(r) { console.log(r) });
...
function addTag(){
	...
	// 傳送資料
	$.post( 'http://chding.es.ncku.edu.tw/Groupack/tag/save.php' , { tid: tid , cid: cid , title: pageInfo.title , content: pageInfo.content , url: pageInfo.url } )
	.fail( function(x) { console.log(x) })
	.done( function(r) { console.log(r) });
...
function delete_card( that ){
	...
	var cid = $(that).parents('.tag').attr( 'sid' ); console.log(cid+' to be deleted');
	
	$.post( 'http://chding.es.ncku.edu.tw/Groupack/tag/delete.php' , { tid: tid , cid: cid })
	.fail( function(x) { console.log(x) })
	.done( function(r) { console.log(r) });
...
function getTagData(){
	...
	$.post( 'http://chding.es.ncku.edu.tw/Groupack/tag/get.php' , { fbid : localStorage.FB_id } )
	.fail( function(x,s,e) { console.log( s + '(' + e + ')' ); })
	.done( function(r) {
		
		// 剖析資料
		var data = JSON.parse(r);
		if( data ) { console.log("成功取得 tag 資料"); }// console.log(data);
		sessionStorage.removeItem('tags_color');
		
		// tags 放到 #inline_all_tags 裡
		for (var i = 0; i < data.length; i++) {
			
			var tag = data[i];
			var div = '\
				<div class="all_tags_inf" t_sid="'+ tag.tid +'" tid="'+ tag.tid +'" t_color="'+ tag.color +'" t_name="'+ tag.name +'" style="background: '+data.color+'"> \
					<div class="all_tags_inf_ModifyTag_area"> \
						<a class="co_a all_tags_inf_ModifyTag" href="#inline_all_tags_inf_ModifyTag">&nbsp;</a> \
					</div> \
					<div class="all_tags_inf_icon"></div> \
					<div class="all_tags_inf_text">'+ tag.name +'</div> \
				</div>';
								
			$('#inline_all_tags .all_tags_ShowAll').after( div );
			$('.all_tags_inf[t_sid='+ tag.tid +']').css('background', tag.color);
			var tags = JSON.parse( sessionStorage.tags_color || '[]' );
			tags.push(tag.color);
			sessionStorage.setItem('tags_color', JSON.stringify(tags));
			
			// cards 放到 #tag_container_CurrentTag 裡
			var cards = tag.cards;
			for (var j = 0; j < cards.length; j++) {
				
				var card = cards[j];
				
				var tag_card = $('.tag[sid='+card.cid+']');
				
				if( tag_card.length === 0 ) { 
					
					// 新 tag card
					div = '\
					<div class="tag" t_sid="'+ tag.tid +'" t_color="'+ tag.color +'" t_name="'+ tag.name +'" sid="'+ card.cid +'">\
						<div class="tag_content">\
							<div class="tag_content_tagname">\
								<span t_sid="'+ tag.tid +'" style="background: '+ tag.color +';">'+tag.name+'</span>\
							</div>\
							<div class="tag_content_color" t_color="'+ tag.color +'" style="background: '+ tag.color +';">&nbsp;</div>\
							<div class="tag_content_selectarea" title="選擇">\
								<div class="tag_content_selectarea_icon"></div>\
							</div>\
							<div class="tag_content_inf"></div>\
							<div class="tag_content_inf_field">\
								<div class="tag_content_inf_field_content" title="群組共享" onclick="showWS()">\
									<div class="post_group">&nbsp;</div>\
									<div class="post_group_text">群組共享</div>\
								</div>\
								<div class="tag_content_inf_field_content" title="加上標籤">\
							...
							<div class="tag_content_area">\
								<div class="show_tag_content_area">\
									<h2 class="co_h2"><a class="co_a click_workspace_cards" href="'+ card.url +'" target="_blank" onclick="show_cardcontain()">'+ card.title +'</a></h2>\
									<p>'+ card.content +'</p>\
									<div class="post_url">\
										<span>'+ card.url +'</span>\
									</div>\
					...
					// tag card 已存在
					var t_sid = $(tag_card).attr('t_sid');
					var t_color = $(tag_card).attr('t_color');
					var t_name = $(tag_card).attr('t_name');
					
					$(tag_card).attr( 't_sid', t_sid+' '+tag.tid ).attr( 't_color', t_color+' '+tag.color ).attr( 't_name', t_name+' '+tag.name );
					
					// 加 tag 到 tag card
 					$(tag_card).find('.tag_content_tagname').append(
						'<span t_sid="'+ tag.tid +'" style="background: '+ tag.color +';">'+tag.name+'</span>'
					);
					$(tag_card).find('.tag_content_color:last').after(
						'<div class="tag_content_color" t_color="'+ tag.color +'" style="background: '+ tag.color +';">&nbsp;</div>'
					);
					
					var count = $(tag_card).find('.tag_content_color').length;
					$(tag_card).find('.tag_content_color[t_color='+ tag.color +']').css('left', count*5-5 );
					
				}
			}
		}	
		var tags = $('div.all_tags_inf');
	});
}
