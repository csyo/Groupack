function close_sharing(){
	...
	$('#sharing_with_group').removeClass('sharing_with_group_on').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
	$('#sharing_with_group_background').addClass('dom_hidden').offset({ top: -1000, left: -1000 });
    ...
    $('._select_card_on').css({'background-color': '', 'color': ''});
    $('._select_folder_area').css({'background-color': '', 'color': ''});
...
$(document).on('click','#sharing_with_group div.sharing_with_group_back',function(){
	...
	document.getElementById('sharing_with_group_showarea_title').innerHTML = '選擇 WorkSpace';
	$('._select_card_on').css({'background-color': '', 'color': ''});
	$('._select_folder_area').css({'background-color': '', 'color': ''}).removeClass('_select_folder_on');
	$('._select_card').removeClass('_select_card_on');
...
$(document).on('click','h2 .fancy_iframe',function(){
    ...
    var pageInfo = { title: title , content: content , url: url };
...
$(document).on('click','#sharing_with_group div.sharing_with_group_submit',function(){
    ...
    if ( select == "選擇 Folder" ) {
        ...
        $.post("http://chding.es.ncku.edu.tw/Groupack/workspace/setCard.php", { fbid: localStorage.FB_id , fid: fID , cid: cID , title: page_info.title , content: page_info.content, url: page_info.url , time: getNow() })
        .done( function(r) { 
			var id = r.indexOf('by'), msg = r.slice(0, id+3)+getName( r.substr( id+3 ) );
			console.log(msg);
...
function showWS(){
    $.post("http://chding.es.ncku.edu.tw/Groupack/workspace/getWS.php", { gid: localStorage.group_selected })
    .done( function(r) { console.log(r)
        ...
        // 顯示按鈕: 新增 (workspace)
        $('.sharing_with_group_addfolder').hide();
        $('.sharing_with_group_addworkspace').show();
        // 加入新截取資料
        var div = '<div class="sharing_with_group_select">';
        for (var i = 0; i < data.length; i++) {
            div = div + '<div class="sharing_with_group_select_field" style="display: block;">\
                  <div class="_select_workspace" data="'+ data[i].wid +'" style="">'+ data[i].wname +'</div>';
            for (var j = 0; j < data[i].folders.length; j++) {
        		div += '<div class="_select_folder" style="display: none;"><div class="_select_folder_area" data="'+ data[i].folders[j].fid +'"style="">'+ data[i].folders[j].fname +'</div></div>';
            }
            div = div+'</div>';
        }
    ...
    .fail( function(x1,x2,x3) { console.log(x3) });
}
