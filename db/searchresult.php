<?php	
	...
	$arySaveKeyword=array(); $arySaveFBID=array(); $aryMix=array(); $zz=0;
	$user_FB_ID=array(); $user_FBNAME=array();
	$groupid = $_POST['sendgroupid'];
	//$logQuery=mysql_query("SELECT SEARCH_KEYWORD,  `FB_ID` FROM  `search_session` WHERE SEARCH_KEYWORD !=  '' GROUP BY SEARCH_KEYWORD");
	if($groupid!=1){
		$logQuery = mysql_query( "SELECT `QueryKeyword`, `UserID` FROM `sessionlog` WHERE `QueryKeyword` != '' AND GroupID = '$groupid' ORDER BY `SID` DESC" );
	}else{
		$logQuery = mysql_query( "SELECT `QueryKeyword`, `UserID` FROM `sessionlog` WHERE `QueryKeyword` != '' ORDER BY `SID` DESC" );
	}
	$sqluser=mysql_query("SELECT `FullName`, `UserID` FROM `userinfo`");
	while ( list($tFullName,$tUserID) = mysql_fetch_row($sqluser) ){
		$user_FBNAME[$zz]=$tFullName;
		$user_FB_ID[$zz]=$tUserID;
		$zz++;
	}
	$zz=0;
	while ( list($tQueryKeyword,$tUserID) = mysql_fetch_row($logQuery) ){
		$arySaveKeyword[$zz]=$tQueryKeyword;
		$arySaveFBID[$zz]=$tUserID;
		$zz++;
	}
	...
	$url = 'https://www.googleapis.com/customsearch/v1?key=AIzaSyA2oggXqL26GRI09JFM4Gt0BULRjmC4Xwk&cx=013036536707430787589:_pqjad5hr1a&q=軟體工程&googlehost=google.com.tw&alt=json&start=1&fields=items(title,link,snippet,formattedUrl)';
    $url_arr = parse_url( $url );
    parse_str( $url_arr['query'], $arr );
    $arr['q'] = urlencode( $_POST['sendkeyword'] );
	$arr['start'] = 1;
	$key_url_now = urldecode( 'https://'.$url_arr['host'].$url_arr['path'].'?'.http_build_query( $arr ) );
	// 宣告陣列變數
    $arrayTitle=array(); $arraySummary=array(); $arrayUrl=array(); $arrayLink=array();
    $aryTitle=array(); $arySummary=array(); $aryUrl=array(); $aryLink=array();
...
if( $aryTitle[0] != null ){
	...
	$old_arr['start'] = 0;
	$key_old_url_now = urldecode( 'http://'.$old_url_arr['host'].$old_url_arr['path'].'?'.http_build_query( $old_arr ) );
