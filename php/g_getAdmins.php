<?php
    ...
    ********************************************/
    ...
    if ( $gid == '' ) {
    	header("錯誤: 沒有傳送 POST 資料");
    	die();
    }
    
    // 取出群組成員資料
    $select = sprintf( "SELECT FB_ID,FB_NAME FROM group_member WHERE G_ID = '%s' AND ROLE = 'ADMIN'", mysql_real_escape_string($gid) );
    $result = mysql_query($select) or die('MySQL failed: '.mysql_error());
    $data = array();
    while ($row = mysql_fetch_assoc($result)) {
        $data['admins'][] =  array( 'fbid' => $row['FB_ID'] , 'fbname' => $row['FB_NAME'] );
    }
